...
int test_nonce(unsigned tnon,hasht mids,datat data,hasht hash,uint32_t* pwdata,uint32_t* pwhash,int chip,int busy,int x,int y)
{
	...
	//fp=fopen("/var/log/bitfury/good.log","a");
	//fprintf(fp,"%d\t%d\t%d\t2\n",chip+1,x,y);
	//fclose(fp);
	return 1;
}
...
#endif
	//fp=fopen("/var/log/bitfury/error.log","a");
...
void chip_init()
{
	FILE* fp=fopen("/opt/bitfury/chip.cnf","r");
	char buf[256],conf[6];
	int c,num,fast;
...
void chip_stat(int chips)
{
	...
	if(first || wait>=5*60) {
		...
		for(c=0;c<chips;c++) {
			...
			if(!first && chipconf[c]) {
				if(good<MINGOOD || miss>MAXERROR*2 || (chipconf[c] & 0x80)) { // fix chip
					chipconf[c] = 0xc0|(chipconf[c] & 0x3F);
				} else {
					chipconf[c] &= 0x7F;
				}
			} // toggle fix signal
		...
		FILE* fp_json=fopen("/var/log/bitfury/stat.json","w");
		...
		if(!first) {
			int c,d=0,x,y;
			FILE* hp=fopen("/var/log/bitfury/hash.log","a");
			...
			fclose(hp);
			hp=fopen("/var/log/bitfury/core.log","w");
			for(c=0;c<chips;c++) {
				d=0;
				for(y=35;y>=0;y--) {
					fprintf(hp,"%d\t",c+1);
			...
			if(record<nr) {
				record=nr;
				system("/bin/cp -f /var/log/bitfury/stat.log /var/log/bitfury/best.log");
			}
		}
		if(wait>=5*60) {
			ctime=time(NULL);
