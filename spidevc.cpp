...
void spi_init(void)
{
	...
	if (spifd < 0) {
		perror("/dev/mem trouble"); exit(1);
	}
	gpio = (volatile unsigned*)mmap(0,SPIBUF,PROT_READ|PROT_WRITE,MAP_SHARED,spifd,0x20200000);
	if (gpio == MAP_FAILED) {
		perror("gpio mmap trouble"); exit(1);
	}
	close(spifd);

	spifd = open("/dev/spidev0.0", O_RDWR);
	if (spifd < 0) {
		perror("Unable to open SPI device"); exit(1);
	}

	if (ioctl(spifd, SPI_IOC_WR_MODE, &mode) < 0) {
		perror("Unable to set WR MODE"); close(spifd); exit(-1);
	}
	if (ioctl(spifd, SPI_IOC_RD_MODE, &mode) < 0) {
		perror("Unable to set RD MODE"); close(spifd); exit(-1);
	}
	if (ioctl(spifd, SPI_IOC_WR_BITS_PER_WORD, &bits) < 0) {
		perror("Unable to set WR_BITS_PER_WORD"); close(spifd); exit(-1);
	}
	if (ioctl(spifd, SPI_IOC_RD_BITS_PER_WORD, &bits) < 0) {
		perror("Unable to set RD_BITS_PER_WORD"); close(spifd); exit(-1);
	}
	if (ioctl(spifd, SPI_IOC_WR_MAX_SPEED_HZ, &maxspeed) < 0) {
		perror("Unable to set WR_MAX_SPEED_HZ"); close(spifd); exit(-1);
	}
	if (ioctl(spifd, SPI_IOC_RD_MAX_SPEED_HZ, &maxspeed) < 0) {
		perror("Unable to set RD_MAX_SPEED_HZ"); close(spifd); exit(-1);
	}
...
void config_reg(int cfgreg, int ena)
{
	...
	if (ena) {
		spi_emit_data(0x7000+cfgreg*32, (const char*)enaconf, 4);
	} else {
		spi_emit_data(0x7000+cfgreg*32, (const char*)disconf, 4);
	}
}
...
void chip_log(int max,int spinow)
{
	...
	fp=fopen("/var/log/bitfury/chip.log","w");
	for(i=0;i<max;i++) {
